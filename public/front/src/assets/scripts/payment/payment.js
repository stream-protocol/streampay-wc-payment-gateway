...
export function getMemo() {
    return window.streampay ? window.streampay.memo : "";
}
...
function setMemo(memo) {
    if (window.streampay && window.streampay.memo) {
        window.streampay.memo = memo;
    }
}
...
function connectWallet(e) {
    if (e.target.id === "streampay-connect-wallet") {
        connectToPhantom();
    }
}
...
async function triggerSolanaPay(e) {
    if (e.target.id === "streampay-place-order-click-layer") {
...
function getRecipientPublicKey() {
    return window.streampay ? .to_public_key ? new PublicKey(window.streampay.to_public_key) : null;
}
...
function setSignatureCookie(signature = "") {
    if (window.Cookies && window.streampay && window.streampay.signature_storage) {
        window.Cookies.set(window.streampay.signature_storage, signature);
    }
}
